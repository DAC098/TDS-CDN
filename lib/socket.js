...
// ----------------------------------------------------------------------------
    ...
    socket.on('request-dir',(dir_path) => {
        let promise = FSM.checkExists(false,root,dir_path);
        promise.then((exists) => {
            if(exists) {
                let promise = FSM.retrieveFolder(path.join(root,dir_path),dir_path);
                promise.then((data) => {
                    socket.emit('request-complete',{type:'dir',data});
                }).catch(() => {
                    socket.emit('request-failed',{type:'dir',msg:'problem when retrieving contents'});
                });
            } else {
                socket.emit('request-failed',{type:'dir',msg:'directory does not exists'});
            }
        });
    });

    socket.on('request-file',(file_path) => {
        let promise = FSM.checkExists(true,root,file_path);
        promise.then((exists) => {
            if(exists) {
                let promise = FSM.retrieveFile(path.join(root,file_path),file_path);
                promise.then((data) => {
                    socket.emit('request-complete',{type:'file',data});
                }).catch(() => {
                    socket.emit('request-failed',{type:'file',msg:'problem when retrieving file'});
                });
            } else {
                socket.emit('request-failed',{type:'file',msg:'file not found'});
            }
        });
    });
        ...
        promise.then((exists) => {
            if(!exists) {
                ...
                promise.then(() => {
                    ...
                    io.emit('update',{type:'dir',path:info.location});
                }).catch(() => {
