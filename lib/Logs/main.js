...
function Logs(options = {}) {
    ...
    const directory = (typeof options.dir !== 'undefined') ?
        (path.isAbsolute(options.dir) ? options.dir : path.join(cwd,options.dir)) :
        path.join(cwd,'log_files');
    ...
    this.appendDebugFile = function appendDebugFile(array) {

    };

    this.appendErrorFile = function appendErrorFile(array) {
    ...
    this.makeDebug = function makeDebug(name) {
        return function(...args) {
            let parent = self;
            if(options.console) {
    ...
    this.makeError = function makeError(name) {
