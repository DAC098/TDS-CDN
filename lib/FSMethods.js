...
exports.removeFile = function removeFile(dir,name) {
    return new Promise(function(resolve,reject) {
        let remove_location = path.join(dir,name);
        log('removing file',remove_location);
        fs.unlink(remove_location,(err) => {
            if(err) {
                log('ERROR: removing file,',remove_location,':',err.message);
                reject();
            } else {
                log('removed file',remove_location);
                resolve();
            }
        });
    });
